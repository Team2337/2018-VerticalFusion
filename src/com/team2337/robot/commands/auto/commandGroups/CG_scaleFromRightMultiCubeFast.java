...
public class CG_scaleFromRightMultiCubeFast extends CommandGroup {


	String ourSwitch, scale;
	public CG_scaleFromRightMultiCubeFast(String ourSwitch, String scale) {
...
//					addSequential(new auaddSequential(new auto_clawOpen());to_liftStop());
					...
					addSequential(new auto_clawOpen());
					addParallel(new auto_driveToAngleWithEncoder(-.7,10,-7,30000,30000,.06,"Left"));
...
//					addSequential(new CG_scorePositionLeftFast());  //score second cube
					...
					addParallel(new auto_driveToAngleWithEncoder(0.5, 2, 0, 26000, 26000, 0.06, "Left"));	//Practice only***********************************************************
...
//						addSequential(new auto_intakeOut(0.4, 0.4));
					...
					//drive to third cube
					addParallel(new auto_driveToAngleWithEncoder(-.6,10,-28,28000,28000,.045,"Left"));
...
//					addSequential(new auto_driveToAngleWithEncoder(.6,10,0,37000,37000,.06,"Left"));
				...
				//Switch is on our side scale is not LRL
					...
					addSequential (new auto_brakeModeOn());
					addSequential (new auto_driveToAngleWithEncoder(.6, 4, 93, 200000, 200000, 0.032, true, 0.02, "Right"));// speed was .5 
					addParallel (new CG_ArmToTop());
					addSequential (new auto_driveToAngleWithEncoder(.6, 4, 95, 296000, 296000, 0.035, true, 0.02, "Right"));// speed was .6 //282000
					//**changed to 303000 from 305000 before q43**//
					addSequential (new auto_driveToAngleWithEncoder(.6, 4, 5, 303000, 303000, 0.04, true, 0.02, "Left"));//276000
					addSequential(new auto_LiftUp(580, 500)); //600, 500 practice
...
//					addParallel (new auto_driveToAngleWithEncoder(-.5, 4, -30, 10000, 10000, 0.04, true));// speed was .9		//Practice only***********************************************************
					addParallel(new auto_driveToAngleWithEncoder(-.6, 4, 0, 10000, 10000, 0.04, "Right"));
					addSequential(new auto_LiftDown(90, 300));
					addSequential(new auto_moveUpperPosition(2850,45)); //100 practice
					addSequential(new auto_wait(0.3));
